...
class Hero:
    def __init__(self, screen, x, y, with_umbrella_filename, without_umbrella_filename):
        ...
        self.x=x
        self.y=y
        self.image_umbrealla=pygame.image.load(with_umbrella_filename)
        self.image_no_umbrealla=pygame.image.load(without_umbrella_filename)
        self.last_hit_time=0
    def draw(self):
        """ Draws this sprite onto the screen. """
        # done 17: Draw (blit) this Hero, at this Hero's position, WITHOUT an umbrella:
        current_immage= self.image_umbrealla

        if time.time()> self.last_hit_time+1:
            current_immage=self.image_no_umbrealla
        self.screen.blit(current_immage, (self.x, self.y))
